...
mkdir -p tmp
file_out=tmp/"$(date +%Y%m%d%H%M)_UK_subtitles.srt"
...
perform_api_call() {
    ...
    curl --request POST \
	 ...
	 -F language="uk" \
         ...
         -o "$part_out"
    echo "SRT Ukrainian Lanugage Output to: $part_out"
}
...
combined_srt="tmp/combined_$(date +%Y%m%d%H%M%S)_UK_Subtitles.srt"
touch "$combined_srt"

if [ "$file_size" -le "$max_size" ]; then
    # File is within the size limit, perform API call
    part_out="tmp/$(date +%Y%m%d%H%M%S)_UK_Subtitles.srt"
    ...
    for (( segment=0; segment<$total_segments; segment++ )); do
        ...
        part_out="tmp/$(date +%Y%m%d%H%M%S)_UK_Subtitles_part${segment}.srt"
